<?php
/**
 * PS SocialPortal Comments List/Form template.
 *
...
<section id="comments" class="article-comments clearfix">
	...
	<?php if ( have_comments() ) : ?>
		...
		<h2 id="comments-title" class="comments-title">
			<?php
			$text = _n( 'Ein Kommentar', '%1$s Kommentare', get_comments_number(), 'social-portal' );

			printf( $text, '<span class="comment-count">'. number_format_i18n( get_comments_number() ) .'</span>'  );
			?>
		</h2>
		...
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
				...
				<nav id="comment-nav-above" class="clearfix navigation comment-navigation comment-navigation-top">
					...
					<h1 class="accessibly-hidden"><?php _e( 'Kommentar Navigation', 'social-portal' ); ?></h1>
		...
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
				...
				<div id="comment-nav-below" class="navigation comment-navigation comment-navigation-bottom">
					...
					<h2 class="accessibly-hidden"><?php _e( 'Kommentar Navigation', 'social-portal' ); ?></h2>
	...
	if ( ! empty( $trackbacks ) ) :
			?>
			<div id="trackbacks" class="article-trackbacks clearfix">
				<h3 class="trackback-title"><?php
					/* translators: %d: trackback count */
                    printf( _n( '%d Trackback', '%d Trackbacks', $num_trackbacks, 'social-portal' ), number_format_i18n( $num_trackbacks ) ) ?>
                </h3>

				<ul id="trackback-list" class="trackback-list">
