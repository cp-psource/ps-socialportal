 ...
 *
 * Modified and adapted for PS SocialPortal by @sbrajesh
 */
class CB_Header_Image_Metabox_Helper {
	...
	public function add_meta_box( $post_type, $post ) {
		...
		foreach ( $allowed_post_types as $post_type ) {
			add_meta_box( 'cb-header-image', __( 'Benutzerdefinierter Header', 'social-portal' ), array(
				$this,
				'render_metabox',
	...
	public function render_metabox( $post ) {
		...
		$header_disabled = get_post_meta( $post->ID, '_cb_hide_page_header', true );
		...
		echo '<p><button class="button primary-button select-image">' . __( 'Bild einstellen', 'social-portal' ) . '</button> <button class="button delete-image">' . __( 'Bild entfernen', 'social-portal' ) . '</button></p>';
	...
	public function update( $post_id ) {
		...
		$this->save_meta_value( $post_id, '_cb_hide_page_header', $header_enabled );
	}
	...
	public function load_js( $hook ) {
		if ( 'post.php' === $hook || 'post-new.php' === $hook ) {
			...
			wp_enqueue_script( 'cb-header-image', social_portal()->url . '/includes/admin/assets/js/custom-header.js', array( 'jquery' ), '', true );
		}
	}
}
