<?php
/**
 * Post Meta Box for allowing Layout Override.
 *
 * @package Communtiy_Builder
 */
...
class CB_Page_Layout_Metabox_Helper {
	...
	public function setup() {
		// add meta box when admin_menu action is called, we may do it on admin_init too.
		add_action( 'add_meta_boxes', array( $this, 'add_meta_box' ) );
		// save the meta value when post is saved.
	...
	public function render_meta_box() {
        ...
        <p>
            <label for="default-<?php echo $key; ?>">
                <input type="radio" name="<?php echo $key; ?>" id="default-<?php echo $key; ?>" <?php checked( 0, $val ) ?> value="0"/>
        ...
        <p>
            <label for="single-col-<?php echo $key; ?>">
                <input type="radio" name="<?php echo $key; ?>" id="single-col-<?php echo $key; ?>" <?php checked( 'page-single-col', $val ) ?> value="page-single-col"/>
        ...
        <p>
            <label for="two-col-right-<?php echo $key; ?>">
                <input type="radio" name="<?php echo $key; ?>" id="two-col-right-<?php echo $key; ?>" <?php checked( 'page-two-col-right-sidebar', $val ) ?> value="page-two-col-right-sidebar"/>
				<?php _e( '2 Spalten (Seitenleiste rechts) Layout', 'social-portal' ); ?>
            </label>
        </p>

        <p>
            <label for="two-col-left-<?php echo $key; ?>">
                <input type="radio" name="<?php echo $key; ?>" id="two-col-left-<?php echo $key; ?>" <?php checked( 'page-two-col-left-sidebar', $val ) ?> value="page-two-col-left-sidebar"/>
				<?php _e( '2 Spalten (Seitenleiste links) Layout', 'social-portal' ); ?>
            </label>
        </p>
		<?php $this->show_page_template_dropdown( $post ); ?>
        <hr/>
        <p><strong><?php _e( 'Layout Blöcke', 'social-portal' ) ?></strong></p>
        <label class="cb-block-item">
            <input type="checkbox" name="_cb_hide_header" value="1" <?php checked( 1, get_post_meta( $post->ID, '_cb_hide_header', true ) ); ?> > <?php _e( 'Header ausblenden', 'social-portal' ); ?>
        </label>

        <label class="cb-block-item">
            <input type="checkbox" name="_cb_hide_page_header" value="1" <?php checked( 1, get_post_meta( $post->ID, '_cb_hide_page_header', true ) ); ?> > <?php _e( 'Banner ausblenden', 'social-portal' ); ?>
        </label>

        <label class="cb-block-item">
            <input type="checkbox" name="_cb_hide_footer" value="1" <?php checked( 1, get_post_meta( $post->ID, '_cb_hide_footer', true ) ); ?> > <?php _e( 'Footer ausblenden', 'social-portal' ); ?>
        </label>

        <label class="cb-block-item">
            <input type="checkbox" name="_cb_hide_footer_copyright" value="1" <?php checked( 1, get_post_meta( $post->ID, '_cb_hide_footer_copyright', true ) ); ?> > <?php _e( 'Footer Copyright-Bereich ausblenden', 'social-portal' ); ?>
        </label>

        <hr/>
        <p><strong><?php _e( 'Panel Links', 'social-portal' ) ?></strong></p>
        <label class="cb-block-item" for="_cb_panel_left_visibility"><?php _ex( 'Sichtbarkeit', 'Einzelpost-Layout-Option', 'social-portal' ); ?></label>
		<?php $this->show_panel_visibility_dropdown( get_post_meta( $post->ID, '_cb_panel_left_visibility', true ), '_cb_panel_left_visibility' ); ?>
        <label class="cb-block-item" for="_cb_panel_left_user_scope"><?php _ex( 'Benutzerbereich', 'Einzelpost-Layout-Option', 'social-portal' ); ?></label>
		<?php $this->show_panel_user_scope_dropdown( get_post_meta( $post->ID, '_cb_panel_left_user_scope', true ), '_cb_panel_left_user_scope' ); ?>

        <hr/>
        <p><strong><?php _e( 'Panel Rechts', 'social-portal' ) ?></strong></p>
        <label class="cb-block-item" for="_cb_panel_right_visibility"><?php _ex( 'Sichtbarkeit', 'Einzelpost-Layout-Option', 'social-portal' ); ?> </label>
		<?php $this->show_panel_visibility_dropdown( get_post_meta( $post->ID, '_cb_panel_right_visibility', true ), '_cb_panel_right_visibility' ); ?>
        <label class="cb-block-item" for="_cb_panel_right_user_scope"><?php _ex( 'Benutzerbereich', 'Einzelpost-Layout-Option', 'social-portal' ); ?> </label>
	...
	/**
     * Show dropdown for selecting page template.
     *
	...
	public function show_page_template_dropdown( $post ) {
        ...
        <label class="screen-reader-text"
               for="page_template"><?php _e( 'Seitentemplate', 'social-portal' ) ?></label>
        <select name="page_template" id="page_template">
			<?php
			$default_title = apply_filters( 'default_page_template_title', __( 'Standard Template', 'social-portal' ), 'meta-box' );
			?>
	...
	/**
     * Show option for enabling/Disabling left,right panel on a page.
     *
	...
	public function save_meta( $post_id ) {
		...
		$keys = array(
			...
			'_cb_hide_page_header'       => '_cb_hide_page_header',
	...
	/**
     * Save associated template details.
     *
	 * @param int $post_id post id.
	 */
	...
	private function get_panel_user_scopes() {
		return array(
			'default'    => __( 'Standort', 'social-portal' ),
