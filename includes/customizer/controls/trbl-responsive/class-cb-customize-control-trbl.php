<?php
/**
 * Top Right Bottom Left(trbl) multi valued Control
 *
...
class CB_Customize_Control_TRBL extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		$css_uri = CB_THEME_URL . '/includes/customizer/controls/trbl-responsive/';
		$js_uri  = CB_THEME_URL . '/includes/customizer/controls/trbl-responsive/';

		wp_enqueue_script(
			'cb-customize-control-trbl-responsive',
			$js_uri . 'trbl-responsive.js',
			array(
				'jquery',
				'customize-base',
			),
		...
		wp_enqueue_style( 'cb-customize-control-trbl-responsive', $css_uri . 'trbl-responsive.css', null, CB_THEME_VERSION );
	}
	...
	public function to_json() {
		...
		$defaults = cb_ensure_responsive_trbl_values( 0 );
		$values   = wp_parse_args( $this->value(), $defaults );

		if ( $values ) {
			$this->linked_choices = count( array_unique( array_values( $values['desktop'] ) ) ) === 1;
		}
                ...
                <div class="input-wrapper cb-trbl-input-wrapper {{ connected_class }}">
                    ...
                    <ul class="cb-trbl-items">
                        ...
                        <li  class='cb-trbl-input-item'>
                            <input type='number' class='cb-trbl-input cb-trbl-responsive' data-id='{{ choiceID }}' value='{{ data.value["desktop"][ choiceID ] }}' data-desktop='{{ data.value["desktop"][ choiceID ] }}' data-tablet='{{ data.value["tablet"][ choiceID ] }}' data-mobile='{{ data.value["mobile"][ choiceID ] }}' {{{ data.inputAttrs }}}>
